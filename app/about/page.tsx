...
import { ReceiptText, Tags, Target, type LucideIcon } from "lucide-react";
...
const PRINCIPLES: { Icon: LucideIcon; title: string; copy: string }[] = [
  ...
  {
    Icon: Tags,
    title: "Paid isn't disqualified. Paid is labeled.",
  ...
  return (
    <div className="flex flex-1 flex-col">
      <main className="mx-auto w-full max-w-[1128px] flex-1 px-4 py-16 sm:px-6 sm:py-20">
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-[1.12fr_0.88fr]">
          ...
          <FramedPhoto
            ...
            alt="The kind of purchase The Verdict Room researches"
          ...
          <div className="mt-13 grid grid-cols-1 gap-5 sm:grid-cols-3">
            {PRINCIPLES.map((p) => (
              <div key={p.title} className="rounded-3xl bg-card px-7 py-7 shadow-[var(--shadow-raised)]">
                <div className="flex size-11 items-center justify-center rounded-2xl bg-well text-primary shadow-[var(--shadow-well)]">
          ...
          <div className="mt-13 grid grid-cols-1 items-center gap-7 rounded-[28px] bg-[linear-gradient(150deg,var(--ink-light),var(--ink))] px-8 py-9 shadow-[var(--shadow-ink)] sm:grid-cols-[1fr_auto] sm:px-10">
            <div>
              ...
              <p className="mt-2.5 max-w-[560px] text-pretty font-serif text-xl leading-relaxed text-ink-foreground">
                ...
                wrong, it&apos;s because we misread the internet, never because someone paid us to.
              </p>
            </div>
